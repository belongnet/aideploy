...
export function buildInternalServiceBaseUrlFromHostPort(
  ...
  if (!isInternalServiceHost(cleanHost)) {
    throw new Error(`${label} must target an internal host`);
  }
  return `http://${cleanHost}:${parsedPort}`;
}

export function resolveAgentInternalHost(template: string, port: number): string {
  const index0 = Math.max(0, port - 8101);
  const index1 = index0 + 1;
  const host = template
    .replace("{index0}", String(index0))
    .replace("{index1}", String(index1));
  if (!host || /[\s/@?#\\]/.test(host) || !isInternalServiceHost(host)) {
    throw new Error("Agent internal host template must resolve to an internal host");
  }
  return host;
}

export function buildAgentInternalUrl(
  template: string,
  port: number,
  path: string,
): string {
  const host = resolveAgentInternalHost(template, port);
  const base = buildInternalServiceBaseUrlFromHostPort(
    host,
    port,
    "Agent internal URL",
  );
